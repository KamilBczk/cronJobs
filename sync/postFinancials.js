...
async function postFinancials(enterprise, accountingDataArr, sql) {
  const financialInDatabase = await sql.query`select guid, reference from cbso.financial where entityId = ${enterprise.guid} AND entityType = 'enterprise'`;
...
async function checkDoubles(referenceData, financialInDatabase, sql) {
  ...
  if (exists)
    ...
    const sameYearRecords = financialInDatabase.recordset.filter(
      (record) => record.reference.startsWith(referenceData.referenceNumberYear)
    );
...
async function addToDatabase(enterprise, accountingData, sql) {
  ...
  requestN.input("exerciseStartDate", sql.DateTime, referenceData.exerciseStartDate);
  requestN.input("exerciseEndDate", sql.DateTime, referenceData.exerciseEndDate);
  ...
  if (groupedData.NM1) {
    ...
    requestNM1.input("reference", sql.VarChar(50), referenceData.referenceNumber);
    ...
    requestNM1.input("exerciseEndDate", sql.DateTime, referenceData.exerciseEndDate);
